<script setup lang="ts">
import { onMounted, nextTick } from 'vue';
import GalleryPageTemplate from "~/components/templates/GalleryPageTemplate.vue";
import * as THREE from 'three';

onMounted(() => {
    nextTick(() => {
        const vertexShader = `
            varying vec2 vUv;
            void main() {
                vUv = uv;
                gl_Position = projectionMatrix * modelViewMatrix * vec4(position, 1.0);
            }
        `;
        const fragmentShader = `
            uniform vec2 u_resolution;
            uniform vec2 u_mouse;
            uniform float u_time;

            varying vec2 vUv;

            void main() {
                float mr = min(u_resolution.x, u_resolution.y);
                vec2 uv = (gl_FragCoord.xy * 2.0 - u_resolution.xy) / mr;

                float d = -u_time * (u_mouse.x * 0.001 + 0.4995) + (u_mouse.y * 0.05 + 0.95);
                float a = 0.0;
                for (float i = 0.0; i < 32.0; ++i) {
                    a += cos(i - d - a * uv.x);
                    d += sin(uv.y * i + a);
                }
                d += u_time * (u_mouse.x * 0.06 + 0.43) + (u_mouse.y * 0.05 + 0.95);
                vec3 col = vec3(cos(uv * vec2(d, a)) * 0.5 + 0.4, cos(a + d) * 0.5 + 0.5);
                col = cos(col * cos(vec3(d, a, 2.5)) * 0.5 + 0.5);
                gl_FragColor = vec4(col,1.0);
            }
        `;

        const scene = new THREE.Scene();
        const camera = new THREE.PerspectiveCamera(75, window.innerWidth/window.innerHeight, 0.1, 1000);
        const renderer = new THREE.WebGLRenderer();

        renderer.setSize(window.innerWidth, window.innerHeight);
        renderer.setPixelRatio(0.5);
        const canvas: HTMLElement | null = document.getElementById('hero__background');
        const cursorChaser: HTMLElement | null = document.getElementById('cursor-chaser');

        if (!canvas) {
            return void alert("Cannot find the hero canvas to draw the background.");
        }

        canvas.appendChild(renderer.domElement);

        const geometry = new THREE.PlaneGeometry(2, 2);
        const uniforms = {
            u_resolution: { type: 'v2', value: new THREE.Vector2(window.innerWidth, window.innerHeight) },
            u_mouse: { type: 'v2', value: new THREE.Vector2() },
            u_time: { type: 'f', value: 0 }
        };

        const material = new THREE.ShaderMaterial({
            vertexShader: vertexShader,
            fragmentShader: fragmentShader,
            uniforms: uniforms
        });

        const plane = new THREE.Mesh(geometry, material);
        scene.add(plane);

        camera.position.z = 0.50;

        function animate() {
            requestAnimationFrame(animate);
            uniforms.u_time.value += 0.02;
            renderer.render(scene, camera);
        }

        animate();

        window.addEventListener('resize', () => {
            const newWidth = window.innerWidth;
            const newHeight = window.innerHeight;
            camera.aspect = newWidth / newHeight;
            // camera.updateProjectionMatrix();
            renderer.setSize(newWidth, newHeight);
            material.uniforms.u_resolution.value.set(newWidth, newHeight);
        });

        window.addEventListener('mousemove', (event) => {
            const normalizedMouseX = event.clientX / window.innerWidth;
            const normalizedMouseY = event.clientY / window.innerHeight;
            material.uniforms.u_mouse.value.set(normalizedMouseX, normalizedMouseY);

            if (cursorChaser) {
                cursorChaser.style.left = `${event.clientX}px`;
                cursorChaser.style.top = `${event.clientY}px`;

                const distanceFromCorners = 0.5 - Math.sqrt(Math.abs(normalizedMouseX - 0.5) * Math.abs(normalizedMouseY - 0.5));

                cursorChaser.style.width = `${distanceFromCorners * 16 + 2}rem`
            }
        });
    });
});
</script>

<template>
    <GalleryPageTemplate>
        <div id="cursor-chaser"></div>

        <header>
            <a id="header__logo">&lt;h1&gt;веб-фабрика&lt;/h1&gt;</a>
        </header>

        <section id="hero">
            <div id="hero__content">
                <h1 id="hero__title">
                    Профессиональная школа брендинга
                </h1>

                <button id="hero__cta">Заказать</button>
            </div>

            <div id="hero__acrylic-filter"></div>
            <div id="hero__background"></div>
        </section>
    </GalleryPageTemplate>
</template>

<style scoped lang="sass">
@import url('https://fonts.googleapis.com/css2?family=Istok+Web:ital,wght@0,400;0,700;1,400;1,700&display=swap')


$grain: url(data:image/png;base64,)
$page-padding: 4rem

*
    font-family: "Inter Tight", system-ui
    user-select: none
    color: black
    cursor: none

#cursor-chaser
    position: fixed
    transform: translateX(-50%) translateY(-50%)
    aspect-ratio: 1
    border: 0.5rem transparentize(cyan, 0.80) solid
    opacity: 0.70
    border-radius: 9999px
    box-shadow: inset 0 0 3rem white, inset 0 0 3rem white, inset 0 0 1rem white, inset 1rem 1rem 2rem white, inset 1rem 1rem 2rem white, inset 1rem 1rem 2rem white
    backdrop-filter: blur(10px) saturate(900%)
    mix-blend-mode: soft-light
    z-index: 30
    display: inline-block
    transition: transform 2000ms ease-in-out

    &:active
        display: inline-block
        pointer-events: none
        transition: transform 200ms ease-in-out
        transform: scale(1.5) translateX(-33%) translateY(-33%)

header, section, footer, #hero__content
    box-sizing: border-box
    padding-inline: $page-padding

header
    position: fixed
    top: 0
    left: 0
    right: 0
    padding-block: 2rem
    z-index: 50

#hero
    position: relative
    contain: content
    padding-inline: 0

#hero__title
    font-size: 6rem
    opacity: 0.60
    font-weight: 1000
    width: max(80dvw, 36rem)
    line-height: 1.6
    mix-blend-mode: overlay
    background-blend-mode: overlay

#hero__acrylic-filter, #hero__content
    position: absolute
    inset: 0
    z-index: 20

#hero__acrylic-filter
    z-index: 10
    background: $grain
    mix-blend-mode: overlay
    background-blend-mode: overlay
    backdrop-filter: blur(4px) saturate(250%)
    display: grid
    place-items: center

#hero__content
    box-shadow: inset 0 0 50dvmin white
    display: flex
    align-items: center
    justify-content: center
    flex-direction: column
    text-align: center

#hero__background
    height: 100%
    width: 100%
</style>